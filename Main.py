...
from SolutionModifier import modify_solution, modify_solution_bruteforce
...
def get_best_modified_solution(solution, modify_solution, **kwargs):
    modified_solutions = [modify_solution(solution[1], **kwargs) for _ in range(best_solution_bees)]
...
def bees_algorithm(iterations, solutions, modify_solution, **kwargs):
    for _ in range(iterations):
        print("Fitness: {}".format(solutions[0][0]))
        selected_spots = solutions[0:selected_spots_number]
        new_solutions = [get_best_modified_solution(s, modify_solution, **kwargs) for s in selected_spots]
...
bees_algorithm(iterations, solutions, modify_solution)

print("BRUTEFORCE ALGO:")

bees_algorithm(iterations, solutions, modify_solution_bruteforce,
               points_number=points_nubmer, distances=distances)
